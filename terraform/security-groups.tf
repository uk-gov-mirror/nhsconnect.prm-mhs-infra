...
resource "aws_security_group_rule" "mhs_inbound_security_group_ingress_test_rule" {
  security_group_id = aws_security_group.mhs_inbound_security_group.id
  type = "ingress"
  from_port = 443
  to_port = 443
...
resource "aws_security_group" "alb_outbound_security_group" {
  ...
  # Allow inbound traffic from test executors
  ingress {
    from_port = 443
    to_port = 443
    protocol = "tcp"
    ...
    description = "Allow inbound HTTPS connections from test host"
  }
...
resource "aws_security_group" "alb_route_security_group" {
  ...
  ingress {
    ...
    to_port = 443
    protocol = "tcp"
    cidr_blocks = [var.testing_subnet]
    description = "Allow inbound HTTPS connections from test hosts"
  }
