...
if [[ -z "$GLOBAL_FORWARD_SERVER" ]]; then
  echo "GLOBAL_FORWARD_SERVER must be set to a DNS server which can reply for internet DNS queries"
  exit 1
fi
    ...
    > /opt/unbound/etc/unbound/unbound.conf << EOT
        server:
            ...
            forward-zone:
                ...
                name: "."
                forward-addr: @GLOBAL_FORWARD_SERVER@

        remote-control:
            control-enable: no
EOT

echo "Configuration written to /opt/unbound/etc/unbound/unbound.conf"
